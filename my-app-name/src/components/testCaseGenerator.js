...
function TestCaseGeneratorPage({ onLogout }) {
  ...
  return (
    <div className="w-full">
      <header className="bg-blue-950 w-full py-6 mb-8 text-center shadow-xl relative">
        <h1 className="text-4xl font-bold tracking-wide uppercase text-white">Automated Test Case Generation</h1>
        <button onClick={onLogout} style={{ position: 'absolute', top: '50%', right: '10px', transform: 'translateY(-50%)' }}
      ...
      <main className="container mx-auto px-4">
        <section className="bg-gray-800 p-8 rounded-xl shadow-lg mb-10">
          <h2 className="text-2xl font-semibold mb-5 text-white">Introduction</h2>
        ...
        <div className="bg-gray-700 p-8 rounded-xl shadow-lg">
          <h2 className="text-2xl font-semibold mb-6 text-white">Enter Text</h2>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Enter text here..."
            className="form-textarea mt-1 block w-full rounded-md border-gray-600 shadow-inner p-4 text-gray-900"
            rows="6"
          ></textarea>
          <button
            onClick={analyzeText}
          ...
          {analysisResult && (
            <div className="mt-4 p-4 bg-gray-800 rounded-xl shadow-inner">
              <h3 className="text-xl font-semibold mb-2 text-white">Analysis Result:</h3>
